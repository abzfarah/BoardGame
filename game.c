        ...
        struct player* computer)
{
	...
	struct player * current = (human->thiscolor == C_WHITE) ? human    : computer;
	struct player * other   = (human->thiscolor == C_WHITE) ? computer : human;

	while(1){
		display_board(board);

		switch(take_turn(current, board)){
			case SUCCESS:
				current->counters++;	//update counter
				if(test_for_winner(board) != G_NO_WINNER)
					return current;	//return winner to main()
				swap_players(&current, &other);	//swap players
				break;
			case FAILURE:
				break;
			case RTM:	//CTRL-D or empty input
				return NULL;	//return to main manu
		}
	}
    return NULL;
}
...
{
	int i,j;
	for(i=BOARDHEIGHT-1; i >= 4; i-- ){
		//check columns
		for(j=0; j < BOARDWIDTH; j++){
			int k, column=0;
			for(k=0; k < 4; k++){
				if(board[i][j] == C_EMPTY)	continue;	//skip empty cells
				if(board[i-k][j] == board[i][j]){
					if(++column == 4)	//if we have 4 in a row
						return (board[i][j] == C_RED) ? G_RED : G_WHITE;
				}
			}
		}

		//check rows
		for(j=0; j < BOARDWIDTH - 4; j++){
			int k, row=0;
			for(k=0; k < 4; k++){
				if(board[i][j] == C_EMPTY)	continue;	//skip empty cells
				if(board[i][j+k] == board[i][j]){
					if(++row == 4)	//if we have 4 in a row
						return (board[i][j] == C_RED) ? G_RED : G_WHITE;
				}
			}
		}
	...
	return (empty_cells == 0) ? G_DRAW : G_NO_WINNER;
}
